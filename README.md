...
- Remote Desktop/Microsoft Remote Desktop for macOS
...
1.) The first thing you are going to want to do is create a virtual machine by going to https://portal.azure.com/. Set up your virtual machine with Windows 10 Pro, version 22H2. Note, that you will want to create a virtual machine with at least 2 vcpus and 16 GB of memory.

2.) Once you have created your virtual machine you will want to connect to it by using the public IP address the VM is setup with. You will connect using the remote desktop connection app. If you are using macOS, you will need to download the Microsoft Remote Desktop application to connect to the VM. Once downloaded, the process is virtually the same.
</p>
<br />
...
<p>
  ...
  Fill out the page as required except for the Database Settings at the bottom. We will get to that. 
...
<p>
  ...
  Once we are connected to the session we will go back to the browser to finish setting everything up. Under the browser's Database Settings, the username will be root and the password will be Password1.
